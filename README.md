...
<sub>▲ 上面这张不是截图贴的素材，而是本项目自身代码渲染导出的真实结果：约 5500 个圆点按疏密铺成山体（点阵由分形噪声山脊 + 8×8 Bayer 有序抖动算出）。</sub>
...
1. **零图片素材**。山体是 `src/lib/dither.js` 用数学算出来的：分形噪声生成四层高低错落的山脊，再用 8×8 Bayer 矩阵做有序抖动，得到每个格子的"墨量"。好处是永远不会有"图裂了"，体积也几乎为零。
2. **山是一片圆点，靠疏密读轮廓**。墨量数据被采样成约 5500 个圆形粒子（`src/lib/particles.js`），明暗完全由点的密度和透明度表现 —— 没有实心色块。它们负责三种行为：**加载时无序漂浮 → 归位**、**被鼠标吹散后弹簧式归位**、**滚动时向两侧飞散**（可逆）。静止时一帧都不重绘：主循环停稳后自行停止，画布留着最后一帧。
3. **不引动画库**。所有动效都是 `IntersectionObserver` + CSS 过渡/rAF 手写的，生产包 gzip 后约 **80 kB**（含 React 本体）。

<div align="center">

<img src="./docs/preview-assemble.png" alt="加载界面上无序漂浮的点、以及归位成山的六个阶段" width="100%">

<sub>▲ 加载过程中点是无序漂浮的（而且就飘在加载界面的纸色底上），加载结束的那一刻开始归位，1.5 秒内全部落到各自的位置</sub>

<img src="./docs/preview-mouse.png" alt="鼠标吹散点阵、弹簧归位、以及滚动时向两侧飞散的连续帧" width="100%">

<sub>▲ 鼠标扫过：附近的点被推开，山上真的空出一块，颗粒堆在四周；移开后弹簧式回弹（会冲过原位再荡回来）。最后两帧是下滑时整片点向两侧飞散</sub>

<sub>上面两张图都由项目自身的代码离线逐帧渲染导出，不是截图。</sub>

</div>

## 特性

- 🏔 **程序化山体** —— 分形噪声 + Bayer 有序抖动算出墨量，零图片素材
- ⚪ **圆点阵轮廓** —— 约 5500 个圆点，明暗由疏密与透明度表现，不用实心色块
- 🎬 **加载时无序漂浮 → 归位** —— 加载界面上点是无序散着的，加载结束瞬间开始归位，1.5 秒内落定、带一点过冲
- 🌬 **鼠标吹散 + 弹簧归位** —— 光标把附近的点推开，山上空出一块；移开后过冲一次再稳住
- 💨 **滚动粒子散开** —— 下滑时整片点向两侧飞散，滚回顶部完全复原（可逆）
- 🖱 **自定义圆圈光标** —— 跟手小黑点 + 带惯性拖尾的圆环，悬停时放大
- 🎭 **开场遮罩** —— `000 → 100%` 进度与方块标志动画，与粒子的归位时刻对齐
- ✨ **滚动揭示** —— 进入视口淡入上浮，逐条错落出现
- 📊 **技能条生长 + 数字滚动**、**卡片悬停 3D 倾斜**、**按钮从左侧填充**
- 📝 **留言便签墙** —— 100 字上限、存在浏览器本地、可单条撕掉
- 📱 **完整响应式** —— 桌面 / 平板 / 手机（手机自动减少粒子数）
- ♿ **尊重系统设置** —— 开启「减少动态效果」后完全不建粒子，直接显示静态点阵图
...
| `SIZE_FACTOR`                              | 粒子直径 = 格子边长 × 它                                   |
| `MOUSE_RADIUS` / `MOUSE_PUSH`              | 鼠标吹散的半径与最大推距                                   |
| `MOUSE_CURL`                               | 推开时的切向旋转分量（0 = 纯径向弹开，0.4 左右像被风卷）   |
| `SPRING` / `DAMPING`                       | 弹簧刚度与阻尼。阻尼调到 0.95 会弹很久，0.85 基本不弹      |
| `INTRO_CLOUD_MS`                           | 无序漂浮持续多久（默认 1500ms，**也是开场遮罩的时长**）    |
| `SETTLE_DURATION_MS` / `SETTLE_STAGGER_MS` | 单个点的归位时长 / 点与点之间的错峰（两者相加正好 1.5 秒） |

散开的"力度、方向、淡出、缩小"在 `particleTransform()` 里；绘制方式（圆点、按透明度
分 10 桶、每桶一次并集填充）在 `components/ParticleMountain.jsx` 的 `draw()` 里。

> 这两个文件里的数学都是**纯函数**（不碰 DOM、不碰 canvas），所以可以直接在 Node 里
> 逐帧跑一遍再渲染成图片来对着调 —— 仓库开发时就是这么验证效果的：
> 无序云、归位、吹散、弹簧过冲、滚动五个状态都是先离线渲染确认过才提交的。
> 顺带一提，点的采样用的是**哈希随机抽取**而不是等间隔抽样：后者会和 Bayer 矩阵的
> 8×8 周期谐振，归位后整片山会浮出一层规则的蜂窝网纹。
...
| **加载时点阵无序漂浮**                    | `components/ParticleMountain.jsx` + `lib/particles.js`  |
| **加载结束瞬间开始归位（1.5 秒内落定）**  | 同上                                                    |
| **鼠标吹散 + 弹簧过冲归位**               | 同上（弹簧在 `lib/particles.js` 的 `stepParticles()`）  |
| **滚动时整片点向两侧散开（可逆）**        | 同上                                                    |
...
- 完整响应 `prefers-reduced-motion: reduce`：所有动画与过渡被压到近乎瞬时，开场遮罩直接跳过，**粒子系统完全不构建**（山景固定为静态位图，也就没有汇聚、吹散、散开）
...
- 山体的本体就是这些点（没有颜色以外的信息依赖），所以即使脚本半途出错，静态点阵位图也仍可作降级显示
