...
- 🗂 **三栏板块选项卡** —— 左侧竖排选项（中文大字 + 小号英文 + 细线，选中项加粗并带一条竖线）+ 中间主内容 + 右侧一块**粒子图形**；点页眉链接、键盘方向键、`#about` 这类锚点都能切，地址栏跟着变（刷新/分享能直达某一栏）
...
- 想调它的大小/位置：`components/FigureCanvas.css` 里 `.tabs__figure` 的 `width`（默认 `min(58vh, 720px)`）和 `right`；它是**背景层**，正文压在它上面
