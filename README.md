...
2. **山是一片圆点，而且每一颗都一模一样大**。墨量数据被采样成约 1.5 万颗点位（`src/lib/particles.js`），**直径统一、颜色统一**—— 明暗完全由点的**疏密**表现，没有实心色块，也没有大小不一的颗粒。它们负责四种行为：**加载时开场那团无序云 → 归位成山**、**被光标按出一个小坑后慢慢流回**、**滚动时向两侧飞散**（可逆），以及**静止时一次 fill 画完的零开销**。
...
<img src="./docs/preview-mouse.png" alt="光标按出小坑、点慢慢流回、以及滚动时向两侧飞散的连续帧" width="100%">

<sub>▲ 光标扫过：只有它底下那一小圈（直径 76px）被按下去，山的其余部分纹丝不动；移开后点**停在原地慢慢流回**，一点也不晃。下面三帧是下滑时整片点向两侧飞散</sub>
...
│  ├─ preview-mouse.png          光标按坑与慢慢流回 + 滚动散开
└─ src/
   ├─ main.jsx                 入口
...
| `INTRO_MS`                        | 开场无序云持续多久（默认 2000ms，**也是开场遮罩的时长**）                                                          |
...
- 完整响应 `prefers-reduced-motion: reduce`：所有动画与过渡被压到近乎瞬时（延时也一起清掉），开场遮罩直接跳过，**粒子系统完全不构建** —— 改用同一套点阵算法画出来的静态图，长得和粒子版一样，只是不会飞
