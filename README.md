...
2. **山是一堆粒子拼的**。同一份墨量数据被采样成约 5000 个独立小方块（`src/lib/particles.js`），它们负责三种行为：**载入时从四面八方汇聚成山**、**被鼠标吹散后弹簧式归位**、**滚动时向两侧飞散**（可逆）。静止时每帧零开销 —— 那时看到的仍是一张静态位图。
3. **不引动画库**。所有动效都是 `IntersectionObserver` + CSS 过渡/rAF 手写的，生产包 gzip 后约 **81 kB**（含 React 本体）。

<div align="center">

<img src="./docs/preview-assemble.png" alt="载入时粒子从四面八方汇聚成山的五个阶段" width="100%">

<sub>▲ 载入时的「汇聚」：粒子从画面外飞入 → 山形从颗粒中析出 → 落定后位图淡入、山体凝实（1.45s → 2.8s）</sub>

<img src="./docs/preview-mouse.png" alt="鼠标吹散山体、离开后弹簧归位的连续帧" width="100%">

<sub>▲ 鼠标移到山体上：用纸色柔边圆盘在粒子层上"擦"出一个空洞，颗粒被推向四周堆在洞边；鼠标离开后粒子弹簧式回弹，会冲过原位再荡回来</sub>

<img src="./docs/preview-particles.png" alt="下滑过程中山体碎成粒子、向两侧散开的六个阶段" width="100%">

<sub>▲ 下滑时山体碎成粒子的六个阶段（进度 0 → 1）</sub>

<sub>以上三张图都由项目自身的代码离线渲染导出，不是截图。</sub>
...
- ✨ **载入时粒子汇聚成山** —— 约 5000 个方块从画面外错峰飞入、带过冲地落位，随后位图淡入凝实
- 🌬 **鼠标吹散 + 弹簧归位** —— 光标把附近粒子连同底下的位图一起"吹开"，留下一个空洞；离开后粒子过冲一次再稳住
- 💨 **滚动粒子散开** —— 滚动时山体碎成粒子向两侧飞散，滚回顶部完全复原（可逆）
...
- ♿ **尊重系统设置** —— 开启「减少动态效果」后动画全部关闭，山景固定为静态图
...
│  ├─ preview-mouse.png          鼠标吹散与弹簧归位
│  └─ preview-particles.png      滚动散开
└─ src/
   ├─ main.jsx                 入口
...
| `ASSEMBLE_DELAY_MS`                       | 载入后多久开始汇聚（默认 1.45s，和开场遮罩滑走对齐）     |
| `ASSEMBLE_DURATION_MS` / `..._STAGGER_MS` | 单个粒子的飞行时长 / 粒子之间的错峰                      |

散开的"力度、方向、淡出、缩小"在 `particleTransform()` 里；位图的裂开与淡出节奏在
`components/ParticleMountain.jsx` 顶部的 `GAP_MAX` / `GAP_FEATHER` / `RASTER_FADE_AT`；
鼠标那个"空洞"的实心半径与收边在 `drawVoid()` 里。

> 这两个文件里的数学都是**纯函数**（不碰 DOM、不碰 canvas），所以可以直接在 Node 里
> 逐帧跑一遍再渲染成图片来对着调 —— 仓库开发时就是这么验证效果的：
> 汇聚、吹散、弹簧过冲、滚动四个状态都是先离线渲染确认过才提交的。
...
| **载入时粒子汇聚成山**                    | `components/ParticleMountain.jsx` + `lib/particles.js`  |
| **鼠标吹散山体 + 弹簧过冲归位**           | 同上（弹簧在 `lib/particles.js` 的 `stepParticles()`）  |
| **滚动时粒子向两侧散开（可逆）**          | 同上                                                    |
