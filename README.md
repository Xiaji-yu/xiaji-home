...
- 🗂 **三栏板块选项卡** —— 左侧竖排选项（中文大字 + 小号英文 + 细线，选中项加粗并带一条竖线）+ 中间主内容（宽度跟屏幕长，620 → 1000px，在屏幕上居中）+ 右侧一块**粒子图形**；图形是背景层、会被正文压住一部分，窄一点的桌面（≤1450px）干脆只留内容。点页眉链接、键盘方向键、`#about` 这类锚点都能切，地址栏跟着变（刷新/分享能直达某一栏）
