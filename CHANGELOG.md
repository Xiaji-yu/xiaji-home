...
- **山景粒子化**：首屏的点阵山会碎成约 5000 个独立小方块（`src/lib/particles.js`）。往下滚动时位图从画面中线柔化裂开并淡出，粒子同时向左右两侧飞散、变小、淡出；进度直接由滚动位置驱动，滚回顶部会完全复原。静止时每帧零开销。
- 鼠标靠近时会推开附近的粒子（与已有的整体视差叠加）
- 手机自动把粒子数降到约 2600 个；系统开启「减少动态效果」时完全不构建粒子，只保留静态山景
